...
async def add_suggestion(event):
    """Users can use this to make suggestions related to the our bots. Their suggestion will be sent to a channel if approved."""
...
async def add_bug(event):
    """Users can use this to report bugs related to the our bots. Their report will be sent to a channel if approved."""
    if not event.is_reply and not event.file and len(event.text.split()) == 1:
        return await event.delete()
    msg = event.message
    if event.is_reply:
