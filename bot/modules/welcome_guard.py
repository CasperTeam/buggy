...
async def welcome(event):
    """Mutes the user upon joining untill the "Click Me" button is pressed. If the user fails to press the button in withing 1000 seconds
    ...
    if chat.id not in config.bot.allowed_chats:
        ...
        try:
            await joined_user.wait_for_confirm(1000)
...
async def confirm_human(event):
    ...
    if data.startswith("verify|"):
        ...
        if str(user.id) != data.split("|")[1]:
            return await event.answer("stop here for further touches you will be warned")
        chat = await event.get_chat()
        ...
        if chat.id in JOINED_USERS[user.id]:
            ...
            if passed.seconds <= 10:
                return await event.answer(f"hmm! too fast. You need to wait {passed.seconds} seconds before pressing the button.")
